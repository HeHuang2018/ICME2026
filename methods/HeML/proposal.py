...
class LayerWiseGramCollector(nn.Module):
    ...
    def collect_all_grams_once(self, x):
        ...
        for idx, layer_info in enumerate(self.adaptable_layers):
            hook = layer_info["module"].register_forward_hook(self._create_hook_handler(idx))
...
def softmax_clamp(logits: torch.Tensor) -> torch.Tensor:
    ...
    return torch.clamp(logits, min=0.0, max=0.99)
...
def collect_params(model):
    ...
    for nm, m in model.named_modules():
        ...
        if isinstance(m, (nn.BatchNorm2d, nn.LayerNorm, nn.GroupNorm)):
            for npn, p in m.named_parameters():
                if npn in ["weight", "bias"]:
                    params.append(p)
                    names.append(f"{nm}.{npn}")
    return params, names
...
class HeML(nn.Module):
    def __init__(self, cfg, model: nn.Module):
        ...
        self.gram_collector = LayerWiseGramCollector(model=self.model, device=self.device, gram_dtype=self.gram_dtype)
        ...
        self.merge_momentum = getattr(self.cfg.ADAPTER.HeML, "ALPHA", 0.9)
    ...
    def forward_and_adapt(self, x: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            pcs = torch.gather(prob_outputs, 1, cls1.view(-1, 1)) - torch.gather(
                prob_outputs_strong, 1, cls1.view(-1, 1)
            )
            pcs = pcs.view(-1).abs()
            _pcs_idx = torch.where(pcs > 0.01)[0]

            pcs_edge = torch.gather(prob_outputs, 1, cls1.view(-1, 1)) - torch.gather(
                prob_outputs_edge, 1, cls1.view(-1, 1)
            )
    ...
    def _map_importance_to_params(self, importance, param_shape, device):
        if len(param_shape) == 1:
            ...
            if param_shape[0] == len(importance):
                return importance.unsqueeze(1).expand(param_shape)
            elif param_shape[1] == len(importance):
                return importance.unsqueeze(0).expand(param_shape)
            else:
                return torch.ones(param_shape, device=device) * 0.5
        else:
            if param_shape[0] == len(importance):
    ...
    def compute_merge_weights_regmean(self, gram, param_shape, layer_idx):
        ...
        importance = (
            1.0 / (torch.diag(regularized_gram) + 1e-8) if self.alpha_regmean != 0.0 else torch.diag(regularized_gram)
        )
    ...
    def _gram_weighted_merge(self, model_to_update, model_to_merge, all_grams, base_momentum, enable_restoration=False):
        ...
        for layer_idx, layer_info in enumerate(self.adaptable_layers):
            ...
            for param_name, param in layer_info["params"].items():
                if param_name not in merge_state_dict:
                    continue
    ...
    @torch.no_grad()
    def gram_guided_merge(self, x: torch.Tensor):
        ...
        if not self.historical_grams:
            ...
            for layer_idx, gram in current_grams.items():
                if layer_idx in self.historical_grams:
                    self.historical_grams[layer_idx] = (1 - self.gram_momentum) * self.historical_grams[layer_idx] + self.gram_momentum * gram
                else:
        ...
        self._gram_weighted_merge(
            ...
            base_momentum=self.merge_momentum,
...
def setup(model: nn.Module, cfg):
    log.info("Setup TTA method: HeML (Hybrid Empirical Model Learning) with Gram Hierarchical Merge")
    model = configure_model(model, cfg)
    return HeML(cfg, model)
